...
//    begin = clock();
    ...
    while(t--)
    {
        ...
        while(ss>>s1)
        {
            ...
            if(s1=="cheater" or s1=="Cheater")
                {s4="charming";
                if(isupper(s1[0]))
                    s4[0]=toupper(s4[0]);
                }
            else if(s1=="kala" or s1=="Kala")   ///programmer cheater kala para na appy nyc lagca
            {
                ss>>s2,ss>>s3;
               // bug1(s2,s3);
                if(s2=="para" and s3=="na")
                    {s4="khela pare";
                    if(isupper(s1[0]))
                    s4[0]=toupper(s4[0]);
                    }
            }
            else if(s1=="appy" or s1=="Appy")
            {
                ss>>s2,ss>>s3;
                if(s2=="nyc" and s3=="lagca")
                    s4="vaiya you are very handsome";
                     if(isupper(s1[0]))
                    s4[0]=toupper(s4[0]);
            }
            else if(s1=="programmer" or s1=="Programmer")
            {
                 s4="man with love and life";
                 if(isupper(s1[0]))
                    s4[0]=toupper(s4[0]);
            }
            else s4=s1;
