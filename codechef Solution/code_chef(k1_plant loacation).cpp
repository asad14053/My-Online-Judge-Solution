...
double x11[5005],y11[5005],a,b,c;
double f(double temp,int n)
{
    double ans=0.0,x,y;
    if(b==0) x=-c/a*1.0,y=temp;
    else if(a==0) x=temp,y=-c/b*1.0;
    else
    {
        x=temp;
        y=(-c-a*x)/b;
    ...
    for(int i=0;i<n;i++)
        ans=ans+(double)sqrt((x-x11[i])*(x-x11[i])+(y-y11[i])*(y-y11[i]));
    return ans;
}
int main()
{
   ...
   // freopen("out0.txt","w",stdout);
    int t,k,i,j,n,m,x,y,q,d;
    cin>>t;
    while(t--)
    {
        cin>>n;
        cin>>a>>b>>c;
        for(i=0;i<n;i++) cin>>x11[i]>>y11[i];
        double low=-5000,high=5000,mid1,mid2;k=200;
        while(k--)
        {
            mid1=low+(high-low)/3.0;
            mid2=high-(high-low)/3.0;

            if(f(mid1,n)>f(mid2,n)) low=mid1;
            else high=mid2;
        }
        printf("%.9f\n",f(low,n));
    }
...
double fun(double m, double a, double b, double c, vll v, vll v1)
{
    double x, y;
    if (b == 0) x = -c / a * 1.0, y = m;
    else if (a == 0) y = -c / b * 1.0, x = m;
    else
    {
        x = a;
        y = (-c - a * x) /b*1.0;
    ...
    double ans = 0.0;
    fr(i, 0, v1.size())
    ans += (double)sqrt((x - v[i]) * (x - v[i]) + (y - v1[i]) * (y - v1[i]));
    return ans;
}
int main()
{
...
//    begin = clock();
    ...
    while (t--)
    {
        ...
        double s = -5000, e = 5000;
        fr(i, 0, 201)
        {
            double l1 = (2.0 * s + e) / 3.0;
            double l2 = (2.0 * e + s) / 3.0;
            if (fun(l1, a, b, c, v, v1) < fun(l2, a, b, c, v, v1))
                e = l2;
            else s = l1;
        }
        printf("%.9f\n", fun(s, a, b, c, v, v1));
