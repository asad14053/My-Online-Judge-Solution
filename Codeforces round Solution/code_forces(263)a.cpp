...
template<typename T>inline bool isinside_grid(T R,T C,T ro,T clm){if((R>=0&&R<ro)&&(C>=0&&C<clm))return true;return false;}
///Direction array
ll X[] = {0, 1, 0, -1};
ll Y[] = {-1, 0, 1, 0};
ll kx[] = { -1, -1, 1, 1, -2, -2, 2, 2 };
ll ky[] = { 2, -2, 2, -2, 1, -1, 1, -1 };
///----------------------Main Code-------------------------------------///
...
//    begin = clock();
    ...
    while(cin>>n)
    {
        ...
        fr(i,0,n)
        {
            ...
            {
                fr(k,0,4)
                if(isinside_grid(i+X[k],j+Y[k],n,n) and v[i+X[k]][j+Y[k]]=='o')
                c++;
                if(c%2==0)
                f&=1;
                else f&=0;
