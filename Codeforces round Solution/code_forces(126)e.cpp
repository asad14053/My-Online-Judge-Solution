...
ll check(ll a,ll b,ll x,ll y,ll z,ll q)
{
    if((q-(a*x+b*y))%z==0)
        return (q-(a*x+b*y))/z;
    return false;
...
//    begin = clock();
    ...
    while (cin >> n>>q)
    {
        ...
        fr(i,0,n)
        {
            ll l1=(2*s+e)/3;
            ll l2=(2*e+s)/3;
            ll l3=check(l1,l2,mp[3],mp[4],mp[5],q);
            if(l3>=0)
            {
                if(mn>((abs(mp[3]*l1-mp[4]*l2)+abs(mp[4]*l2-mp[5]*l3))))
                {
                    mn=((abs(mp[3]*l1-mp[4]*l2)+abs(mp[4]*l2-mp[5]*l3)));
                    a=l1;
                    b=l2;
                    c=l3;
                    e=l2;
