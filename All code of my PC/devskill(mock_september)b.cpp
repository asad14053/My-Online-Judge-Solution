...
ll getPairsCount(ll arr[], ll n, ll sum)
{
    ll c = 0,*y;
    fr(i, 0, n/2+1)
    {
       bug1(*(arr+i+1),*(arr+n-1));
        y=find(arr+i+1,arr+n-1,(sum-(arr[i]%sum)));

        if(*y>=*(arr+i+1) and *y<*(arr+n-1))
         c++,bug2(*y,sum-(arr[i]%sum),arr[i]),arr[i]=-1e18;
    }
    return c;
}
int main()
