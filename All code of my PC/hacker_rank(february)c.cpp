...
inline bool isinside_grid(int R,int C,int ro,int clm){if((R>=0&&R<ro)&&(C>=0&&C<clm))return true;return false;}
int main()
{
#ifndef ONLINE_JUDGE
   input();
...
//    begin = clock();
    ...
    while (t--)
    {
     ...
     //   a=7-a;
     //   bug1(a,b);
     //   bug(isinside_grid((int)a+x8[0],(int)b+y8[0],8,8));
         c=0;
        fr(i,0,8)
        {
        //  bug1((int)a+x8[i],(int)b+y8[i]);
          if(isinside_grid((int)a+x8[i],(int)b+y8[i],8,8))
          {
              if(v[a+x8[i]][b+y8[i]]=='N' or (v[a+x8[i]][b+y8[i]]=='P' and a+x8[i]==7))
              c++;
          }
        }
        x=a-1,y=b-1;
         f=0;
        while(isinside_grid(x,y,8,8))
        {
          
          if(isupper(v[x][y]))
            {
                if(v[x][y]!='K' and v[x][y]!='P'  and islower(v[x][y]))
                {
                  if(v[x][y]=='B' or v[x][y]=='Q' and !f)
                    c++;
                  f=1;
                }
                else if(v[x][y]=='P' and x==7 and !f)
                  c+=2;
            }
          x--;
          y--;
