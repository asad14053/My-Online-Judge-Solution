...
void dfs(ll x, ll y, ll c)
{
  ...
  for (int i = 0; i <2; i++)
  {
    if (BOUNDARY(x + X[i], y + Y[i]) && v[x][y]==1 && v[x + X[i]][y + Y[i]] == 1 && !visited[x + X[i]][y + Y[i]])
    {
...
//    begin = clock();
  ...
  while (cin >> row >> column)
  {
   ...
   //   bug(v[i][j]);
    ...
    //   // c=v[i-1][j];
    //   fr(i, 1, n)
    //   {  mx=max(v[i - 1][j],v[i][j]);
    //     if (v[i - 1][j] > 0 and v[i][j] > 0)
    //      mx=max(v[i][j],mx)+1,bug1(i,j);
    //    }
    //      mx=max(c,mx);
    // }
    fr(i, 0, row)
