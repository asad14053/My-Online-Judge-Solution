...
int  LeftButtonPressed;//vaeiable to store the information about the
left
button pressd
int  RightButtonPressed; //vaeiable to store the information about the
right button pressd
...
//class ICONS
//THIS CLASS IS THE ONE WHICH DRAWS ALL THE SHAPES WHICH U CAN SEE ON
THE
//ON THE LEFT HAND SIDE OF THE SCREEN ON THE BUTTONS
//PLZ DONOT TRY TO CHANE THE COLOR OR THE COORDINATE OF THE MEMBER
FUNCTIONS
...
{
	setlinestyle(0, 1, 1);
	//from here to line no 588 the arrrow is drawn which u can see on the
open i_con
	setcolor(BLACK);
...
void ICONS::Save_ico(int x1, int y1)
{
  ...
  //the upper white portion is the result of the following staement
	...
	//this state ment results in the dark gray part which is on the bottom
of
the
	//icon is created
	setfillstyle(1, DARKGRAY);
...
********//
//this function draws the line icon which is the simplest of the all
the
...
//*******   Start of the finction void Rectangle_ico(int x1, int
y1)********//

//this is another simple function  it just draws the rectangle which is
on
the
...
{       setlinestyle(0,1,1);
	...
	setfillstyle(1,RED);
	floodfill(x1+19,y1+12,BLACK);
	for(int i=8;i<=12;i+=2)
	{
	for(int j=9;j<=11;j+=2)
	{
	putpixel(x1+i-1,y1+j-2,YELLOW);}}
