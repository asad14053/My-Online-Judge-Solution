...
///I/O
#define input()             freopen("G:/daily practise/in0.txt","r",stdin)
#define output()            freopen("G:/daily practise/out0.txt","w",stdout)
  ...
  //  cost[x][y]=0;
    while(!Q.empty())
    {
        ...
        for(k=0; k<4; k++)
        {
            vx = ux+X[k];
            vy = uy+Y[k];
            if((vx>=0&&vx<R) and (vy>=0&&vy<C) and g[vx][vy]!='@' and vis[vx][vy]==false)
            {
                
                if(color[vx][vy]==-1)
                {
                	color[vx][vy]=(color[ux][uy]==0)?1:0;
                	vis[vx][vy]=true;
                    Q.push(vx);
                    Q.push(vy);
...
//    begin = clock();
    ...
    while (cin>>n>>m)
    {
        ...
        if(bfs(0,0))
            cout<<"Bi-colorable"<<endl;
        else cout<<"Not Bi-colorable"<<endl;
        //fr(i,0,n)
        //{
        //    fr(j,0,m)
        //    cout<<color[i][j];
