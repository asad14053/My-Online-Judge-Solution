...
template<typename T>inline vs vs_parse(T str)
{
    vs res;
    string s;
...
template<typename T>inline T pw(T b,T p)
{
    T r=1;
    if(b==0)return 0;
    fr(i,0,p)r*=b;
...
{
    if (b == 0)return 1;
    else if (b % 2 == 0)return sqr(bigmod(a, b / 2, m)) % m;
    else return (a % m*bigmod(a, b - 1, m)) % m;
}
template<typename T> inline T modinv(T n,T mod)
{
...
///Inside grid
template<typename T>inline bool isinside_grid(int R,int C,int ro,int clm)
{
...
//    begin = clock();
    ...
    while(cin>>a>>b)
    {
        ...
        else if(ceil((double)a/b)==(ll)a/b)
        {
            if(a/b<10)
            {
                cout<<-1<<endl;
