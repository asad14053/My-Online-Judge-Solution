...
int main() //https://www.hackerearth.com/challenge/competitive/july-circuits-18/algorithm/special-shop-69904c91/
{
#ifndef ONLINE_JUDGE
  input();
  output();
...
//    begin = clock();
    ...
    {
      cin>>n>>a>>b;
      x=((n*(max(a,b)))/(double)(a+b));
      ll mn=inf;
      ll l=(max(0LL,x-5));
      ll p=(x+6);
      fr(i,l,p)
      mn=min(min(a,b)*i*i+max(a,b)*(n-i)*(n-i),mn);
      out(mn);
      //cout<<n<<endl;
    }
//    end = clock();
