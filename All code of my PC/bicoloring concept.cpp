...
///----------------------Main Code-------------------------------------///
///Inside grid
template<typename T>inline bool isinside_grid(T R,T C,T ro,T clm)
{
    if((R>=0&&R<ro)&&(C>=0&&C<clm))return true;
    return false;
}
template<typename T>inline void print_grid(T GRID,ll ro,ll clm)
{
    fr(i,0,ro)
    {
        fr(j,0,clm)cout<<GRID[i][j]<<" ";
        puts("");
    }
}
ll X4[] = {0, 1, 0, -1};
...
void bfs(ll x,ll y)
{
    ...
    while(!q.empty())
    {
       ...
       // bug1(w,z);
        fr(i,0,4)
        {
           // bug3(isinside_grid(w+X4[i],z+Y4[i],n,m),w+X4[i],z+Y4[i],vis[w+X4[i]][z+Y4[i]]);
            if(isinside_grid(w+X4[i],z+Y4[i],n,m) and !vis[w+X4[i]][z+Y4[i]])
            {
               // bug2(w+X4[i],z+Y4[i],vis[w+X4[i]][z+Y4[i]]);
                vis[w+X4[i]][z+Y4[i]]=1;
                ...
                if(v[w+X4[i]][z+Y4[i]] =='.')
                {
                    ...
                    if(v[w][z]=='B')
                        v[w+X4[i]][z+Y4[i]]='W';
                    else v[w+X4[i]][z+Y4[i]]='B';
                    //dfs(x+X4[i],y+Y4[i]);
                }
                q.push(mp(w+X4[i],z+Y4[i]));
            }
        }
        q.pop();
